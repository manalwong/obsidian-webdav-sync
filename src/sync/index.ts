...
import { RemoteWebDAVFileSystem } from '~/fs/remote-webdav'
...
export class GenericWebDAVSync {
	...
	constructor(
		...
		this.remoteFs = new RemoteWebDAVFileSystem({
			vault: this.options.vault,
			remoteBaseDir: this.options.remoteBaseDir,
			webdavUrl: this.plugin.settings.webdavUrl,
			account: this.plugin.settings.account,
			credential: this.plugin.settings.credential,
		})
	...
	async start({ mode }: { mode: SyncStartMode }) {
		try {
			...
			emitSyncError(error)
			logger.error('Sync error:', errMsg)
		} finally {
