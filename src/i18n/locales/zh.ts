export default {
	...
	settings: {
		...
		webdavUrl: {
			name: 'WebDAV 服务器地址',
			desc: '输入 WebDAV 服务器地址（例如 https://dav.example.com/dav/）',
		},
		language: {
			name: '语言',
			desc: '选择界面语言',
		...
		cache: {
			...
			dumpName: '缓存管理',
			dumpDesc:
				'插件会在您的设备上保存远程文件夹的目录信息。当您更换设备时，这些信息会丢失，需要在首次同步前重新获取。如果您的文件数量较多，获取过程可能较慢。导出功能可将这些信息保存到 WebDAV 服务器，导入功能则可将数据恢复到新设备，使您无需等待即可直接同步。',
			dump: '导出',
			restoreName: '导入缓存',
			restoreDesc:
			...
			clearName: '清除本地缓存',
			clearDesc:
				'删除当前设备上的本地缓存数据。此操作无法撤销，会导致下次同步前需要重新建立缓存。仅影响当前设备，不会影响 WebDAV 服务器上的数据。',
			clear: '清除',
			confirm: '确认清除',
			cleared: '缓存已成功清除',
			clearModal: {
	...
	sync: {
		...
		suggestUseClientForManyTasks:
			'提示：当同步任务较多时，首次同步可能需要较长时间，请耐心等待。',
		modalTitle: '同步进行中',
		cancelButton: '取消同步',
		progressText: '正在同步文件',
		fileOp: {
